...
<div
    ...
    @else
        @if($status === 'up')
            <x-ui.icons.arrow-up class="w-[24px] h-[24px] bg-[#C0F7B4]"/>
        @elseif ($status === 'down')
            <x-ui.icons.arrow-down class="w-[24px] h-[24px] bg-[#FECDD3]"/>
        @else
            <x-ui.icons.minus class="w-[24px] h-[24px] bg-[#F1F5F9]"/>
        @endif
    @endif
</div>
